...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        console.error('Erreur PUT TypeTc:', error);
        if (error.code === 'P2025') {
...
export async function DELETE(req: Request, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        console.error('Erreur DELETE TypeTc:', error);
        if (error.code === 'P2025') {
            return NextResponse.json({ error: 'Élément introuvable ou non autorisé' }, { status: 404 });
        }
